...
def loss(x, i, label_list, main_set):
...
def iterational_optimization(optim_fun: callable, ll):
    for i, _ in enumerate(ll):
        # optimize one label
        optim_fun(loss, bounds=[(-20, 20), (-20, 20), (0, 4)], args=(i, ll, m))


def test_fun(fun: callable):
    ll = [Label((0, 0), (100, 100), 5, 2) for _ in range(lnum)]
    start = time.time()
    iterational_optimization(fun, ll)
    end = time.time()
    print(end-start)
    present(ll)

test_fun(dual_annealing)
test_fun(differential_evolution)
